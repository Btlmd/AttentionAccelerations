...
from models.efficient_attention import MultiheadAttention, add_nested_argument
from models.efficient_attention.attn_utils import (
...
class LinearRA(MultiheadAttention):
    def __init__(self, 
                ...
                num_landmarks=49,
                kernel_size=None,
                proposal_gen='pool-mixed',
                use_antithetics=False,
                use_multisample=False,
                pool_module_type='dense',
                mis_type='mis-opt',
                alpha_coeff=1.0,
                *args,
                **kwargs):
        super(LinearRA, self).__init__(dim=32*2, num_heads=2, *args, **kwargs)
    ...
    def _proposal_gen_1d(self, q, k, v, key_padding_mask=None):
    ...
    def forward(self, q, k, v, key_padding_mask=None):
        ...
        q_bar, k_bar, q, k, v = self._proposal_gen_1d(q, k, v, key_padding_mask)
        mu = q_bar + k_bar
    ...
    def add_attn_specific_args(parent_parser, struct_name="attn_args", prefix=""):
        ...
        add_nested_argument(parser, '--{}pool-module-type'.format(_name_prefix), struct_name=struct_name, prefix=prefix, default='dense', type=str)
        add_nested_argument(parser, '--{}mis-type'.format(_name_prefix), struct_name=struct_name, prefix=prefix, default='mis-opt', type=str)
        add_nested_argument(parser, '--{}proposal-gen'.format(_name_prefix), struct_name=struct_name, prefix=prefix, default='pool-mixed', type=str)
